...
{                               /* --- main function for testing */
  ...
  DIM     C     = 0;            /* tile size for caching */
  int     P     = proccnt();    /* number of threads */
  int     mode  = FCM_PCC;      /* computation mode */
  int     valid = 0;            /* flag for result validation */
  long    S     = 0;            /* seed value for random numbers */
  ...
  if (C <  0) C = V;            /* get the tile size for caching */
  if (P <  0) P = 0;            /* and the number of threads */
  if (S <= 0) S = time(NULL);   /* get the seed value and */
  ...
  if (valid) {                  /* if to validate the results */
    ...
    if (C == 0) {               /* if on demand computation */
      fprintf(stderr, "test (on-demand, fcm_get) ... ");
      mode |= FCM_CACHE;        /* add the cache flag */
      fcm = SFXNAME(fcm_create)(data, V, T, mode, C);
    ...
    else if (C < V) {           /* if to use a (partial) cache */
      fprintf(stderr, "test (%s, fcm_next) ... ",
              (mode & FCM_JOIN) ? "cr+join" : "blk+sig");
      mode |= FCM_CACHE|FCM_THREAD; /* add cache and thread flag */
      fcm = SFXNAME(fcm_create)(data, V, T, mode, C, P);
      if (!fcm) error(E_NOMEM); /* create functional connect. matrix */
      diff = 0;                 /* initialize the difference flag */
      for (t = SFXNAME(fcm_first)(fcm); t > 0;
           t = SFXNAME(fcm_next) (fcm)) {
  ...
  /* --- test performance --- */
    ...
    if (C == 0) {               /* if on demand computation */
      fprintf(stderr, "perf (on-demand, fcm_get) ... ");
      t0 = timer();             /* start the timer */
      mode |= FCM_CACHE;        /* add the cache flag */
      fcm = SFXNAME(fcm_create)(data, V, T, mode, C);
      if (!fcm) error(E_NOMEM); /* create functional connect. matrix */
      for (DIM i = 0; i < SFXNAME(fcm_dim)(fcm); i++)
        for (DIM j = i+1; j < SFXNAME(fcm_dim)(fcm); j++)
          a = SFXNAME(fcm_get)(fcm,i,j);
      SFXNAME(fcm_delete)(fcm); /* delete the func. connect. matrix */
      ...
      fprintf(stderr, "perf (%s, fcm_next) ... ",
              (mode & FCM_JOIN) ? "cr+join" : "blk+sig");
      t0 = timer();             /* start the timer */
      mode |= FCM_CACHE|FCM_THREAD; /* add cache and thread flag */
      fcm = SFXNAME(fcm_create)(data, V, T, mode, C, P);
